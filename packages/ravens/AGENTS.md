...
- Running `bun dev` from `packages/ravens` starts the live interactive TUI. Do not run it as a blocking foreground command when you need to inspect the result.
- Start it in `tmux` instead: `tmux new-session -d -s ravens-dev 'bun dev'`.
- Capture the current TUI output with: `tmux capture-pane -pt ravens-dev`.
- Stop the session explicitly when done: `tmux kill-session -t ravens-dev`.
...
export class Service extends Context.Service<Service, Interface>()("@ravens/Foo") {}
...
The Memory system provides persistent cross-session memory for the agent.

## Architecture

- `src/memory/schema.ts` — MemoryEntry, MemoryCategory, MemoryContext, SaveInput, SearchInput
- `src/memory/store.ts` — Markdown file CRUD (MemoryStore Service)
- `src/memory/memory.ts` — High-level Memory Service (load/save/search/remove)
- `src/tool/memory-save.ts` — `memory_save` tool
- `src/tool/memory-search.ts` — `memory_search` tool
- `src/session/system.ts` — Injects memory into System Prompt via `<memory>` tags
- `src/session/compaction.ts` — Protects `<memory>` sections from compaction

## Categories

| Category | Scope | Path | Committed? |
|----------|-------|------|------------|
| user | Global | `~/.config/colime/memory/user.md` | No (personal) |
| project | Per-project | `<project>/.colime/memory/project.md` | Yes (shared) |
| feedback | Per-project | `<project>/.colime/memory/feedback.md` | No (personal) |
| reference | Per-project | `<project>/.colime/memory/reference.md` | Yes (shared) |

## Token Budget

Total 2000 tokens: user 500, project 800, feedback 400, reference 300.
Overflow trims from lowest priority (reference → feedback → project → user).

## Compaction Protection

- `PRUNE_PROTECTED_TOOLS` includes `memory_save` and `memory_search`
- `<memory>...</memory>` sections are replaced with `[memory context available in system prompt]` during compaction

## Testing

Memory tests live in `test/memory/`:
- `schema.test.ts` — Schema validation
- `memory.test.ts` — User category store tests
- `store-project.test.ts` — Project/feedback/reference category tests (uses `testEffect` + `it.instance`)
- `memory-service.test.ts` — High-level service tests
- `compaction-memory.test.ts` — Compaction protection tests

Project-level store tests require `InstanceState.context` — use `it.instance` from `testEffect` pattern (see `test/lib/effect.ts`).
