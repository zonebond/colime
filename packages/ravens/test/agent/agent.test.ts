...
function withExperimentalScout<A, E, R>(enabled: boolean, self: Effect.Effect<A, E, R>) {
  return Effect.acquireUseRelease(
    Effect.sync(() => {
      const original = Flag.RAVENS_EXPERIMENTAL_SCOUT
      Flag.RAVENS_EXPERIMENTAL_SCOUT = enabled
      return original
    }),
    () => self,
    (original) =>
      Effect.sync(() => {
        Flag.RAVENS_EXPERIMENTAL_SCOUT = original
      }),
  )
}
...
it.instance("plan agent denies edits except .ravens/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".ravens/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".ravens", "skill", "perm-skill")
