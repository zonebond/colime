...
describe("ServerAuth", () => {
  test("does not emit auth headers without a password", () => {
    ...
    Flag.RAVENS_SERVER_USERNAME = "alice"

    expect(ServerAuth.header()).toBeUndefined()
    expect(ServerAuth.headers()).toBeUndefined()
  })

  test("defaults to the ravens username", () => {
    Flag.RAVENS_SERVER_PASSWORD = "secret"
    Flag.RAVENS_SERVER_USERNAME = undefined
  ...
  test("validates decoded credentials against effect config", () => {
    ...
    expect(ServerAuth.authorized({ username: "ravens", password: Redacted.make("secret") }, config)).toBe(false)
  })
})
