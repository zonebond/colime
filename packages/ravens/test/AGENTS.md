...
- `config?: Partial<Config.Info>` - Write a `ravens.json` config file
...
- Directories are created in the system temp folder with prefix `ravens-test-`
...
it.instance("uses the temp directory", () =>
  Effect.gen(function* () {
    ...
    expect(test.directory).toContain("ravens-test-")
  }),
)
```
