...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "ravens.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "ravens.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "ravens.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "ravens.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["ravens.json", "ravens.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "ravens.json"),
        path.join(tmp.path, "ravens.jsonc"),
        path.join(project, "ravens.json"),
        path.join(project, "ravens.jsonc"),
      ])
    })
  })
