...
const writeConfig = Effect.fn("test.writeConfig")(function* (dir: string, config: Partial<Config.Info>) {
  yield* writeText(
    ...
    JSON.stringify({ $schema: "https://ravens.ai/config.json", ...config }),
  )
})
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      yield* llm.text("packages/ravens/src/session/processor.ts")
  ...
  () =>
    Effect.gen(function* () {
      ...
      const other = yield* prompt.prompt({
        sessionID: session.id,
        agent: "build",
        model: { providerID: ProviderID.make("ravens"), modelID: ModelID.make("kimi-k2.5-free") },
        noReply: true,
        parts: [{ type: "text", text: "hello" }],
      })
