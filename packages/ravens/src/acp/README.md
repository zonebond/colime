...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for ravens.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal ravens sessions
...
ravens acp --cwd /path/to/project
```
...
```bash
RAVENS_ENABLE_QUESTION_TOOL=1 ravens acp
```
...
- **Terminal Integration**: Full terminal support via ravens' bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | ravens acp
```
...
ACP sessions map cleanly to ravens' internal session model:
