...
const PRUNE_PROTECTED_TOOLS = ["skill", "memory_save", "memory_search"]

/** Strip <memory>...</memory> sections from text content to prevent compaction from summarizing persistent memory. */
function stripMemorySections(text: string): string {
...
- Do not repeat information from <memory> sections — it is provided separately and persists across sessions.`
type Turn = {
  start: number
  end: number
  id: MessageID
...
export class Service extends Context.Service<Service, Interface>()("@ravens/SessionCompaction") {}
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const processCompaction = Effect.fn("SessionCompaction.process")(function* (input: {
      ...
      // Strip <memory> sections from text parts to prevent compaction LLM from summarizing persistent memory
      for (const msg of msgs) {
        for (const part of msg.parts) {
          if (part.type === "text" && typeof part.text === "string") {
            part.text = stripMemorySections(part.text)
          }
        }
      }
