...
export class Service extends Context.Service<Service, Interface>()("@ravens/SessionPrompt") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const insertReminders = Effect.fn("SessionPrompt.insertReminders")(function* (input: {
      ...
      if (!Flag.RAVENS_EXPERIMENTAL_PLAN_MODE) {
      ...
      return yield* Effect.uninterruptibleMask((restore) =>
        Effect.gen(function* () {
          ...
          const { msg, part, cwd } = yield* Effect.gen(function* () {
            ...
            return { msg, part, cwd: session.directory }
      ...
      function* (sessionID: SessionID) {
        ...
        while (true) {
          ...
          if (
            ...
            // Persist preview from the first non-synthetic text part of the last assistant message
            if (lastAssistantMsg) {
              const textParts = lastAssistantMsg.parts.filter((p) => p.type === "text") as MessageV2.TextPart[]
              const previewPart = textParts.find((p) => !p.synthetic && p.text?.trim())
              if (previewPart?.text) {
                yield* sessions
                  .setPreview({ sessionID, preview: previewPart.text.slice(0, 200) })
          ...
          const outcome: "break" | "continue" = yield* Effect.gen(function* () {
            ...
            const [skills, env, instructions, memoryCtx, modelMsgs] = yield* Effect.all([
              sys.skills(agent),
              sys.environment(model, session.directory),
