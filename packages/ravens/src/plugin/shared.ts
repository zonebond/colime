...
export async function checkPluginCompatibility(target: string, ravensVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(ravensVersion) || semver.major(ravensVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.ravens
  if (typeof range !== "string") return
  if (!semver.satisfies(ravensVersion, range)) {
    throw new Error(`Plugin requires ravens ${range} but running ${ravensVersion}`)
  }
}
