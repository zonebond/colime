...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".ravens"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".ravens", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".ravens", "tui.json"),
  }
}
...
async function load(input: { api: Api; config: TuiConfig.Resolved; dispose?: () => void }) {
  ...
  runtime = next
  try {
    const records = Flag.RAVENS_PURE ? [] : (config.plugin_origins ?? [])
    if (Flag.RAVENS_PURE && config.plugin_origins?.length) {
