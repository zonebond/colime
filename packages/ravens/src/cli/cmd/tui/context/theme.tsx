...
import ravens from "./theme/ravens.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@ravens-ai/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "ravens")
        draft.active = typeof active === "string" ? active : "ravens"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.ravens, store.mode)
    })
