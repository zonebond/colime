...
const TUI_SCHEMA_URL = "https://ravens.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from ravens.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const ravens = await ravensFiles(input)
  for (const file of ravens) {
...
async function ravensFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "ravens"),
    ...(await Filesystem.findUp(["ravens.json", "ravens.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "ravens"))
  }
  if (Flag.RAVENS_CONFIG) files.push(Flag.RAVENS_CONFIG)
