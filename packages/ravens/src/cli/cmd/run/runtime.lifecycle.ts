...
export async function createRuntimeLifecycle(input: LifecycleInput): Promise<Lifecycle> {
  return withRunSpan(
    "RunLifecycle.boot",
    {
      "ravens.agent.name": input.agent,
      "ravens.directory": input.directory,
      "ravens.first": input.first,
      "ravens.model.provider": input.model?.providerID,
      "ravens.model.id": input.model?.modelID,
      "ravens.model.variant": input.variant,
      ...
      try {
        ...
        const close = async (next: {
          ...
          return withRunSpan(
            "RunLifecycle.close",
            {
              "ravens.show_exit": next.showExit,
