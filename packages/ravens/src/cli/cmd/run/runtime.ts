...
async function runInteractiveRuntime(input: RunRuntimeInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "ravens.mode": input.resolveSession ? "local" : "attach",
      "ravens.initial_input": !!input.initialInput,
      "ravens.demo": input.demo,
    },
    async (span) => {
      ...
      setRunSpanAttributes(span, {
        "ravens.directory": ctx.directory,
        "ravens.resume": ctx.resume === true,
        "ravens.agent.name": state.agent,
        "ravens.model.provider": state.model?.providerID,
        "ravens.model.id": state.model?.modelID,
        "ravens.model.variant": state.activeVariant,
      ...
      const shell = await createRuntimeLifecycle({
        ...
        onModelSelect: async (model) => {
          ...
          setRunSpanAttributes(span, {
            "ravens.model.provider": model.providerID,
            "ravens.model.id": model.modelID,
            "ravens.model.variant": state.activeVariant,
          })
          return {
...
export async function runInteractiveLocalMode(input: RunLocalInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "ravens.directory": input.directory,
      "ravens.initial_input": !!input.initialInput,
      "ravens.demo": input.demo,
    },
    async () => {
