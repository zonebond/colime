...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for ravens serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(RavensHttpApi)
...
export const PublicApi = RavensHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "ravens",
    version: "1.0.0",
    description: "ravens api",
