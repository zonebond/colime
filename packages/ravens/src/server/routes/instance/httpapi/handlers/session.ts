...
export const sessionHandlers = HttpApiBuilder.group(InstanceHttpApi, "session", (handlers) =>
  Effect.gen(function* () {
    ...
    const list = Effect.fn("SessionHttpApi.list")(function* (ctx: { query: typeof ListQuery.Type }) {
      ...
      const labelSvc = yield* Label.Service
      const labelMap = yield* labelSvc.getSessionLabels(sessions.map((s) => s.id))
      for (const s of sessions) {
        const lbl = labelMap.get(s.id)
        if (lbl) {
          ;(s as any).labelId = lbl.labelId
          ;(s as any).labelName = lbl.labelName
        }
      }
      return sessions
    })
    ...
    const get = Effect.fn("SessionHttpApi.get")(function* (ctx: { params: { sessionID: SessionID } }) {
      const s = yield* SessionError.mapStorageNotFound(session.get(ctx.params.sessionID))
      const labelSvc = yield* Label.Service
      const label = yield* labelSvc.getSessionLabel(ctx.params.sessionID)
      if (label) {
        ;(s as any).labelId = label.id
        ;(s as any).labelName = label.name
      }
      return s
    })
    ...
    const create = Effect.fn("SessionHttpApi.create")(function* (ctx: { payload?: Session.CreateInput }) {
      ...
      if (ctx.payload?.labelId) {
        const labelSvc = yield* Label.Service
        yield* labelSvc.setSessionLabel(result.id, ctx.payload.labelId)
        const label = yield* labelSvc.get(ctx.payload.labelId)
        if (label) {
          ;(result as any).labelId = label.id
          ;(result as any).labelName = label.name
        }
      }
      return result
    })
    ...
    const update = Effect.fn("SessionHttpApi.update")(function* (ctx: {
      ...
      if (ctx.payload.labelId !== undefined) {
        const labelSvc = yield* Label.Service
        yield* labelSvc.setSessionLabel(ctx.params.sessionID, ctx.payload.labelId)
      }
