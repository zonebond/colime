...
import { Module } from "@ravens-ai/core/util/module"
...
export const Vue: Info = {
  ...
  async spawn(root) {
    ...
    const args: string[] = []
    if (!binary) {
      if (Flag.RAVENS_DISABLE_LSP_DOWNLOAD) return
...
export const JDTLS: Info = {
  ...
  async spawn(root) {
    ...
    const dataDir = await fs.mkdtemp(path.join(os.tmpdir(), "ravens-jdtls-data"))
    return {
      process: spawn(
        java,
        [
