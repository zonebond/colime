...
import { sanitizedProcessEnv } from "@ravens-ai/core/util/ravens-process"
...
export class Service extends Context.Service<Service, Interface>()("@ravens/Ripgrep") {}
...
function filesArgs(input: FilesInput) {
  const args = ["--no-config", "--files", "--glob=!.git/*", "--glob=!node_modules/*", "--glob=!.pnpm-store/*"]
...
function searchArgs(input: SearchInput) {
  const args = ["--no-config", "--json", "--hidden", "--glob=!.git/*", "--glob=!node_modules/*", "--glob=!.pnpm-store/*", "--no-messages"]
  if (input.follow) args.push("--follow")
  if (input.glob) {
    for (const glob of input.glob) args.push(`--glob=${glob}`)
  }
    ...
    Effect.gen(function* () {
      ...
      const tree: Interface["tree"] = Effect.fn("Ripgrep.tree")(function* (input: TreeInput) {
        ...
        for (const file of list) {
          if (file.includes(".ravens")) continue
