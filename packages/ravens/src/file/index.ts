...
import { NonNegativeInt, type DeepMutable } from "@ravens-ai/core/schema"
...
export interface Interface {
  ...
  readonly readStream: (file: string) => Stream.Stream<Uint8Array, Error>
...
export class Service extends Context.Service<Service, Interface>()("@ravens/File") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const readStream: Interface["readStream"] = (file: string) =>
      Stream.unwrap(
        Effect.gen(function* () {
          const ctx = yield* InstanceState.context
          const full = path.join(ctx.directory, file)
          ...
          if (!containsPath(full, ctx)) {
            return Stream.fail(new Error("Access denied: path escapes project directory"))
          }

          const exists = yield* appFs.existsSafe(full)
          if (!exists) {
            return Stream.fail(new Error(`File not found: ${file}`))
          }

          return appFs.stream(full).pipe(
    ...
    log.info("init")
    return Service.of({ init, status, read, readStream, list, search })
  }),
)
