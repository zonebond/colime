...
import { NonNegativeInt, PositiveInt, type DeepMutable } from "@ravens-ai/core/schema"
...
function normalizeLoadedConfig(data: unknown, source: string) {
  ...
  delete copy.tui
  log.warn("tui keys in ravens config are deprecated; move them to tui.json", { path: source })
  return copy
}
...
export const Info = Schema.Struct({
  ...
  server: Schema.optional(ConfigServer.Server).annotate({
    description: "Server configuration for ravens serve and web commands",
  }),
  command: Schema.optional(Schema.Record(Schema.String, ConfigCommand.Info)).annotate({
    description: "Command configuration, see https://ravens.ai/docs/commands",
  }),
  ...
  ).annotate({ description: "Agent configuration, see https://ravens.ai/docs/agents" }),
...
// Uses the shared `DeepMutable` from `@ravens-ai/core/schema`. See the definition
...
export class Service extends Context.Service<Service, Interface>()("@ravens/Config") {}

function globalConfigFile() {
  const candidates = ["ravens.jsonc", "ravens.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://ravens.ai/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      ...
      if (!Flag.RAVENS_CONFIG && !Flag.RAVENS_CONFIG_DIR && !Flag.RAVENS_CONFIG_CONTENT) {
        const file = globalConfigFile()
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://ravens.ai/config.json" }, null, 2))
            .pipe(Effect.catch(() => Effect.void))
        }
      }
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "config.json")))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "ravens.json")))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "ravens.jsonc")))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://ravens.ai/config.json"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "RAVENS_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            log.debug("fetching remote config", { url: `${url}/.well-known/ravens` })
            const response = yield* Effect.promise(() => fetch(`${url}/.well-known/ravens`))
            if (!response.ok) {
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://ravens.ai/config.json"
            const source = `${url}/.well-known/ravens`
        ...
        if (Flag.RAVENS_CONFIG) {
          yield* merge(Flag.RAVENS_CONFIG, yield* loadFile(Flag.RAVENS_CONFIG))
          log.debug("loaded custom config", { path: Flag.RAVENS_CONFIG })
        }

        if (!Flag.RAVENS_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("ravens", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.RAVENS_CONFIG_DIR) {
          log.debug("loading config from RAVENS_CONFIG_DIR", { path: Flag.RAVENS_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void, never>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".ravens") || dir === Flag.RAVENS_CONFIG_DIR) {
            for (const file of ["ravens.json", "ravens.jsonc", "config.json"]) {
          ...
          // Auto-discovered plugins under `.ravens/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.RAVENS_CONFIG_CONTENT) {
          const source = "RAVENS_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.RAVENS_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          log.debug("loaded custom config from RAVENS_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["RAVENS_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("RAVENS_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (existsSync(managedDir)) {
          for (const file of ["ravens.json", "ravens.jsonc"]) {
        ...
        if (Flag.RAVENS_PERMISSION) {
          result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.RAVENS_PERMISSION))
        }
        ...
        if (Flag.RAVENS_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.RAVENS_DISABLE_PRUNE) {
