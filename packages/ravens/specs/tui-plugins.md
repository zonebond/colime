...
- Author package entrypoint is `@ravens-ai/plugin/tui`.
...
- Import types from `@ravens-ai/plugin/tui`.
- `@ravens-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.
...
When `plugin` entries exist in a writable `.opencode` dir or `OPENCODE_CONFIG_DIR`, OpenCode installs `@ravens-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@ravens-ai/plugin/tui`.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@ravens-ai/plugin/tui`.
