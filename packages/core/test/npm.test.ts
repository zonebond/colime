...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@ravens/acme")).toBe("@ravens/acme")
    expect(Npm.sanitize("@ravens/acme@1.0.0")).toBe("@ravens/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/ravens/acme.git"
    const expected = win ? "acme@git+https_//github.com/ravens/acme.git" : spec
